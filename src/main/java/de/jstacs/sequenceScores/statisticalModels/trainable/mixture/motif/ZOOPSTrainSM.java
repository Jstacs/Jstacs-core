...
import de.jstacs.sequenceScores.statisticalModels.trainable.mixture.motif.positionprior.UniformPositionPrior;
...
public class ZOOPSTrainSM extends HiddenMotifMixture {
	 ...
	 *            {@link Algorithm#GIBBS_SAMPLING} the
	 ...
	 *            <code>algorithm == {@link Algorithm#GIBBS_SAMPLING}</code>
	...
	protected ZOOPSTrainSM( TrainableStatisticalModel motif, TrainableStatisticalModel bg, boolean trainOnlyMotifModel, int starts, double[] componentHyperParams,
		...
		super( new TrainableStatisticalModel[]{ motif, bg },
	 ...
	 *             if the <code>models</code> can not be cloned
	 * 
	 * @see ZOOPSTrainSM#ZOOPSTrainSM(TrainableStatisticalModel, TrainableStatisticalModel, boolean, int, double[], double[], PositionPrior, Algorithm, double, TerminationCondition,  Parameterization, int, int, BurnInTest)
	 * @see Algorithm#EM
	 */
	...
	protected void setTrainData( DataSet data ) throws Exception {
		...
		boolean rev = model[0] instanceof StrandTrainSM;
	...
	/* (non-Javadoc)
	 * @see de.jstacs.sequenceScores.statisticalModels.trainable.mixture.AbstractMixtureTrainSM#createSeqWeightsArray()
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see de.jstacs.sequenceScores.statisticalModels.trainable.mixture.AbstractMixtureTrainSM#getNewWeights(double[], double[], double[][])
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see de.jstacs.sequenceScores.statisticalModels.trainable.mixture.motif.HiddenMotifMixture#getMinimalSequenceLength()
	 */
	@Override
    ...
    public double[] getStrandProbabilitiesFor( int component, int motif, Sequence sequence, int startpos ) throws Exception {
        if( component == 0 && motif == 0 ) {
            if( model[0] instanceof StrandTrainSM ) {
                Sequence help = sequence.getSubSequence(startpos, model[0].getLength());
                double[] logProbs = {
                     ( (StrandTrainSM)model[0] ).getLogProbFor( 0, help ),
                     ( (StrandTrainSM)model[0] ).getLogProbFor( 1, help )
                };
...
//TODO remove
    	...
    	for( int modelIndex = 0; modelIndex < model.length; modelIndex++ ) {
    		if( model[modelIndex] instanceof AbstractMixtureTrainSM ) {
    			( (AbstractMixtureTrainSM)model[modelIndex] ).setTrainData( sample[modelIndex] );
    			usedWeights[modelIndex] = ( (AbstractMixtureTrainSM)model[modelIndex] ).createSeqWeightsArray();
			}
    	}
...
//TODO end
    	...
    	TrainableStatisticalModel[] backup = ArrayHandler.clone(model);
	...
	protected double iterate( int start, double[] dataWeights, MultivariateRandomGenerator m, MRGParams params[] ) throws Exception {
		...
		switch( algorithm ) {
			case EM:
				...
				double[][] help = null;
				TrainableStatisticalModel[] backup = null;
				double best, current;
